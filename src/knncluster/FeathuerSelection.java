...
public class FeathuerSelection {
	...
	public void getFeather(String path){
		...
		//
		//C:/Program Files/eclipse/workspace/DocCluster/knncluster/
		String abolutepath="/Users/zhangzhen/gitRepository/DocCluster/src/knncluster/";
		...
		for(i=0;i<dirlist.length;i++){
			...
			File subdir=new File(abolutepath+path+File.separator+dirlist[i]);
			String []subdirlist=subdir.list();
			System.out.println(subdirlist.length);
			for(j=0;j<subdirlist.length;j++){
				arraylist.add(dirlist[i]+File.separator+subdirlist[j]);
			}
		}
	...
	//	feathermatrix=new double[list.length][feathercount];
		...
		for( i=0;i<list.length;i++){
			...
			String filedata=fileToStr(path+File.separator+list[i]);
